...
void CPrepareStatement::SetParam(uint32_t index, const char *value)
{
    if (index >= m_param_cnt)
    {
        log_error("index too large: %d\n", index);
    ...
    m_param_bind[index].buffer_type = MYSQL_TYPE_LONG_BLOB;
    m_param_bind[index].buffer = (char *)value;
    m_param_bind[index].is_null = 0;
    m_param_bind[index].length = NULL;
}
...
bool CPrepareStatement::ExecuteUpdate(char *buff ,int index,unsigned long length)
{
    if (!m_stmt)
    {
        log_error("no m_stmt\n");
    ...
    if (mysql_stmt_send_long_data(m_stmt,index, buff, length+1))
    {
        log_error(" err : %s\n", mysql_stmt_error(m_stmt));
        return false;
    }
...
MYSQL_STMT* CPrepareStatement::getStmt()
{
    return this->m_stmt;
}
